...
const TYPE_COLORS: Record<NodeType, string> = {
  ...
  "front-matter": "bg-slate-200 text-slate-800",
  toc: "bg-slate-200 text-slate-800",
  chapter: "bg-primary text-primary-foreground",
  "learning-goals": "bg-amber-200 text-amber-900",
  discover: "bg-sky-200 text-sky-900",
  learn: "bg-emerald-200 text-emerald-900",
  methods: "bg-teal-200 text-teal-900",
  assess: "bg-violet-200 text-violet-900",
  integration: "bg-fuchsia-200 text-fuchsia-900",
  "prior-knowledge": "bg-lime-200 text-lime-900",
  summary: "bg-yellow-200 text-yellow-900",
  exercises: "bg-orange-200 text-orange-900",
  deepen: "bg-rose-200 text-rose-900",
  ict: "bg-cyan-200 text-cyan-900",
  situation: "bg-pink-200 text-pink-900",
  guidance: "bg-indigo-200 text-indigo-900",
  solution: "bg-green-200 text-green-900",
  "numbered-lesson": "bg-blue-200 text-blue-900",
...
function CurriculumViewerPage() {
  ...
  // Auto-select first chapter after parse
  useEffect(() => {
  ...
  const exportJson = (mode: "full" | "structure") => {
    if (!parsed) return;
    const data = toJSON(parsed.root, { includeContent: mode === "full" });
      ...
      <header className="border-b border-border bg-card">
        ...
        {parsed && (
          <div className="mx-auto flex max-w-[1400px] flex-wrap items-center gap-x-6 gap-y-1 px-6 pb-3 text-xs text-muted-foreground">
            ...
            <span>عدد العُقد: <b className="text-foreground">{parsed.stats.totalNodes}</b></span>
            ...
            <span className="flex flex-wrap gap-1">
              {Object.entries(parsed.stats.byType).map(([t, n]) => (
                <span
                  key={t}
                  className={`rounded px-1.5 py-0.5 ${TYPE_COLORS[t as NodeType] ?? ""}`}
                >
                  {TYPE_LABELS_AR[t as NodeType] ?? t}: {n}
                </span>
              ))}
            </span>
          </div>
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground">
          سطر {node.startLine + 1}–{node.endLine + 1} · مستوى {node.level}
        </span>
      </div>
      ...
      {tab === "json" && (
        <pre dir="ltr" className="overflow-auto rounded bg-muted p-4 text-xs">
          {JSON.stringify(toJSON(node), null, 2)}
        </pre>
      )}
    </div>
  );
